...
import { Search, Plus, Loader2, AlertCircle, ArrowUp, ArrowDown, Check, ChevronDown, FolderOpen, Target, ChevronRight } from 'lucide-react';
...
const ClaimsList = () => {
  ...
  const sortedAndFilteredClaims = claims
    .filter(claim => {
      ...
      return claim.client_name?.toLowerCase().includes(searchLower) ||
  ...
  return (
    <div className="p-3 sm:p-4 md:p-8 min-h-screen page-enter">
      ...
      <div className="mb-4 sm:mb-6 md:mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 animate-fade-in-up">
        <div>
          <div className="flex items-center gap-2 sm:gap-3 mb-1 sm:mb-2">
            <img src={NAV_ICONS.garden} alt="Garden" className="w-10 h-10 sm:w-12 sm:h-12 object-contain icon-3d-shadow" />
            <h1 className="text-xl sm:text-2xl md:text-3xl font-tactical font-bold text-white tracking-wide text-glow-orange">THE GARDEN</h1>
          </div>
          <p className="text-zinc-500 font-mono text-xs sm:text-sm uppercase tracking-wider">Track and manage all property claims</p>
        </div>
        <button 
      ...
      <div className="card-tactical p-3 sm:p-4 md:p-5 mb-4 sm:mb-6 animate-fade-in-up" style={{ animationDelay: '0.1s' }}>
      ...
      {error && (
        <div className="mb-4 md:mb-6 bg-red-500/10 border border-red-500/30 rounded-lg p-4 flex items-center gap-2 text-red-400">
          ...
          <button onClick={fetchClaims} className="ml-auto text-sm text-zinc-400 hover:text-orange-400">
            Retry
          </button>
        </div>
      )}
      ...
      <div className="card-tactical p-5 animate-fade-in-up" style={{ animationDelay: '0.15s' }}>
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-3">
            ...
            <span className="font-tactical font-bold text-white uppercase text-sm tracking-wide">{sortedAndFilteredClaims.length} Targets</span>
          </div>
        ...
        {loading ? (
          <div className="space-y-3">
            ...
            <div className="text-center py-16">
              ...
              <h3 className="text-lg font-tactical font-bold text-white mb-2 uppercase">No Targets Found</h3>
              <p className="text-zinc-500 mb-6 max-w-sm mx-auto font-mono text-sm">Initialize your first mission to begin operations</p>
              <button 
                onClick={() => navigate('/claims/new')}
                  ...
                  <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-wrap items-center gap-2 mb-3">
                        <span className="font-tactical font-bold text-base text-white group-hover:text-orange-400 transition-colors">{claim.claim_number}</span>
                        <span className={`px-2 py-0.5 rounded text-[10px] font-mono uppercase ${getStatusBadge(claim.status)}`}>{claim.status}</span>
                        <span className={`px-2 py-0.5 rounded text-[10px] font-mono uppercase ${getPriorityBadge(claim.priority)}`}>{claim.priority}</span>
                      </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4 mb-3">
                        ...
                        <div className="min-w-0">
                          <p className="text-[10px] font-mono text-zinc-600 uppercase mb-1">Location</p>
                      ...
                      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-zinc-500 font-mono">
                        <span>Policy: <span className="text-zinc-400">{claim.policy_number}</span></span>
                        <span>Updated: <span className="text-zinc-400">{formatDate(claim.updated_at)}</span></span>
                      </div>
                    </div>
                    ...
                    <div className="text-left md:text-right flex-shrink-0 flex items-center gap-3">
                      <div>
                        ...
                        <p className="text-[10px] text-zinc-600 font-mono uppercase mt-1">Est. Value</p>
                      </div>
