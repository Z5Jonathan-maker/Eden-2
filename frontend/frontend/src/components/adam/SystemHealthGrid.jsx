        ...
        return (
          <div 
            ...
            className={`card-tactical p-4 border-l-2 ${
              isOperational ? 'border-l-green-500' : isDegraded ? 'border-l-yellow-500' : 'border-l-red-500'
            }`}
          >
            ...
            <Badge className={`${
              isOperational ? 'bg-green-500/20 text-green-400' : 
            ...
            {comp.latency_ms && (
              <p className="text-xs text-zinc-500 mt-1 font-mono">{comp.latency_ms.toFixed(0)}ms</p>
            )}
          </div>
        );
      })}
