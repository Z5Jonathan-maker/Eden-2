"""
Eden Incentives Engine - API Routes
...
Phase 2: Rule Evaluation & Leaderboards
...
SEED_METRICS = [
    ...
    },
    {
        ...
        "description": "Appointments scheduled from canvassing",
        "source_collection": "harvest_visits",
        "filter_query": {"status": "AP"},
        "aggregation": "count",
        "icon": "📅",
        "unit": "appts",
        "format": "integer",
    ...
    },
    {
        ...
        "description": "Contracts signed on the spot",
        "source_collection": "harvest_visits",
        "filter_query": {"status": "SG"},
        "aggregation": "count",
        "icon": "✍️",
        "unit": "contracts",
        "format": "integer",
    ...
    },
    {
        ...
        "source_collection": "harvest_score_events",
        "source_field": "final_points",
        "aggregation": "sum",
        "icon": "⭐",
        "unit": "pts",
        "format": "integer",
    ...
    },
    {
        ...
        "description": "Total contract value from signed deals",
    ...
    },
    {
        "id": "metric-installs",
        "slug": "installs",
        "name": "Installs Completed",
        "description": "Completed installation jobs",
        "source_collection": "installs",
        "aggregation": "count",
        "icon": "🔧",
        "unit": "installs",
        "format": "integer",
    ...
    },
    {
        ...
        "description": "Customer reviews obtained",
    ...
    },
    {
        ...
        "description": "Customer referrals obtained",
        "source_collection": "referrals",
        "aggregation": "count",
        "icon": "🤝",
        "unit": "referrals",
        "format": "integer",
    ...
    },
    {
        ...
        "slug": "close_rate",
        "name": "Close Rate",
        "description": "Percentage of appointments that become contracts",
        "source_collection": "harvest_visits",
        "aggregation": "ratio",
        "numerator_metric_id": "metric-contracts",
        "denominator_metric_id": "metric-appointments",
        "icon": "📊",
        "unit": "%",
        "format": "percentage",
        "is_system": True,
...
async def get_metrics(
    ...
    metrics = await db.incentive_metrics.find(query, {"_id": 0}).to_list(100)
...
async def create_metric(
    ...
    """Create a custom metric (admin only)"""
    if current_user.get("role") not in ["admin"]:
        raise HTTPException(status_code=403, detail="Admin access required")
    
    # Check slug uniqueness
    existing = await db.incentive_metrics.find_one({"slug": data.slug})
    if existing:
        raise HTTPException(status_code=400, detail=f"Metric with slug '{data.slug}' already exists")
    
    metric_id = f"metric-{data.slug}"
    ...
    await db.incentive_metrics.insert_one(metric_doc)
    
    return {"id": metric_id, "message": "Metric created successfully"}
...
async def seed_metrics() -> list:
    ...
    for metric in SEED_METRICS:
        ...
        metric["supports_individual"] = True
        metric["supports_team"] = True
        metric["supports_company"] = True
        
        await db.incentive_metrics.update_one(
            {"id": metric["id"]},
            {"$set": metric},
            upsert=True
        )
