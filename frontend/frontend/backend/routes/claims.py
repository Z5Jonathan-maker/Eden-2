...
FILENAME_TYPE_RULES = [
    ...
    (["coverage determination", "coverage_determination", "coverage decision", "coverage_decision"], "coverage_determination"),
...
def _categorize_filename(filename: str) -> str:
    ...
    for patterns, doc_type in FILENAME_TYPE_RULES:
        if any(p in fn for p in patterns):
            # Special case: "report" without "photo" -> inspection_report
            return doc_type
    if "report" in fn.lower() and "photo" not in fn.lower():
        return "inspection_report"
    return "carrier_correspondence"


@router.post("/categorize-all-documents", dependencies=[Depends(require_permission("documents.create"))])
async def categorize_all_documents(
    current_user: dict = Depends(get_current_active_user),
):
    """Batch-categorize all gmail_attachment documents by filename pattern matching."""
    try:
        ...
        if not docs:
            return {"message": "No uncategorized documents found", "updated": 0}
        ...
        # Add document inventory note to each claim
        notes_added = 0
        for cid, cats in claim_summaries.items():
            lines = [f"[Document Inventory] Synced from Gmail ({sum(cats.values())} docs):"]
        ...
        logger.error(f"Categorize documents error: {e}")
