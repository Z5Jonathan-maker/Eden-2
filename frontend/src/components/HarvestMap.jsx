...
import { MapPin, Navigation, Plus, X, Check, Phone, Calendar, Home, Ban, RefreshCw, Loader2, ChevronDown, ChevronUp } from 'lucide-react';
...
const PinPopup = ({ pin, onLogVisit, onClose, userPosition, dispositions = DEFAULT_PIN_STATUSES }) => {
  ...
  return (
    <div className="min-w-[200px]">
      {pin.address && (
        <div className="font-medium text-sm mb-2 text-gray-900">{pin.address}</div>
      )}
...
export const HarvestMap = ({ territoryId = null, className = '', onPinStatusChange, activeFilters = null }) => {
  const { position, accuracy, error: gpsError, refreshPosition, hasLocation } = useGpsWatch();
  const { pins, loading, error: pinsError, createPin, logVisit, fetchPins, dispositions } = useHarvestPins({ territoryId });
  ...
  const handleLogVisit = useCallback(async (pin, status) => {
      ...
      <div className="absolute bottom-24 left-4 right-4 z-[1000] bg-white/95 backdrop-blur rounded-lg shadow-lg p-3">
        <div className="flex items-center justify-between text-xs">
          <div className="flex items-center gap-3">
            <span className="font-medium text-gray-700">{filteredPins.length} pins{activeFilters && activeFilters.length < 6 ? ' (filtered)' : ''}</span>
            <div className="flex items-center gap-1">
              <div className="w-3 h-3 rounded-full" style={{ backgroundColor: dispositions.SG?.color || '#10B981' }} />
              <span>{filteredPins.filter(p => p.status === 'SG').length}</span>
            </div>
            <div className="flex items-center gap-1">
              <div className="w-3 h-3 rounded-full" style={{ backgroundColor: dispositions.AP?.color || '#3B82F6' }} />
              <span>{filteredPins.filter(p => p.status === 'AP').length}</span>
            </div>
            <div className="flex items-center gap-1">
              <div className="w-3 h-3 rounded-full" style={{ backgroundColor: dispositions.CB?.color || '#8B5CF6' }} />
