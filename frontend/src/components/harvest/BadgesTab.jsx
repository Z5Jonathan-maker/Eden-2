      ...
      <div className="grid grid-cols-3 gap-3 sm:gap-4">
        {hasBadges ? (
          ...
          DEFAULT_BADGES.map((badge, i) => (
            <div key={i} className="text-center p-3 sm:p-4 rounded-xl bg-zinc-800/50 border border-zinc-700/30 opacity-60 grayscale">
...
const BadgeCard = ({ badge }) => {
  ...
  return (
    ...
    >
      <div className={`w-14 h-14 sm:w-16 sm:h-16 mx-auto mb-2 rounded-full flex items-center justify-center text-2xl sm:text-3xl ${
        earned 
      ...
      {!earned && badge.points_bonus > 0 && <p className="text-amber-400/60 text-xs mt-2">+{badge.points_bonus} pts</p>}
      {badge.rarity && (
        <Badge className={`mt-2 text-[10px] ${RARITY_STYLES[badge.rarity] || RARITY_STYLES.common}`}>
          {badge.rarity}
        </Badge>
      )}
    </div>
