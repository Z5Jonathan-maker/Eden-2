...
import { ArrowLeft, PlayCircle, CheckCircle, Clock, BookOpen, Award, ChevronRight, FileText, AlertCircle } from 'lucide-react';
...
function CourseDetail() {
  ...
  function getProgress() {
    ...
    var completed = course.user_progress.completed_lessons ? course.user_progress.completed_lessons.length : 0;
    return Math.round((completed / course.lessons.length) * 100);
  }

  function allLessonsComplete() {
    if (!course || !course.lessons || !course.user_progress || !course.user_progress.completed_lessons) return false;
  ...
  return (
    <div className="p-8 bg-gray-50 min-h-screen">
      <div className="mb-6">
        <Button variant="ghost" onClick={function() { navigate('/university'); }} className="mb-4">
        ...
        <div className="mt-4">
          ...
          <div className="w-full bg-gray-200 rounded-full h-3">
            <div className="bg-orange-600 h-3 rounded-full" style={{ width: getProgress() + '%' }}></div>
          </div>
        </div>
      </div>
          ...
          <Card>
            ...
            <CardContent className="p-0">
              <div className="divide-y">
                {course.lessons && course.lessons.map(function(lesson, index) {
                  ...
                  return (
                    <div
                      ...
                      onClick={function() { setActiveLesson(lesson); setShowQuiz(false); }}
                    >
                      <div className="flex items-center">
                        ...
                        <div>
                          <p className={isActive ? 'font-medium text-orange-600' : 'font-medium'}>{lesson.title}</p>
                ...
                {course.quiz && course.quiz.length > 0 && (
                  <div
                    ...
                    onClick={function() { if (allLessonsComplete()) { setShowQuiz(true); setActiveLesson(null); } }}
                  >
                    <div className="flex items-center">
                      ...
                      <div>
                        <p className={showQuiz ? 'font-medium text-orange-600' : 'font-medium'}>Final Quiz</p>
              ...
              <CardContent>
                ...
                <div className="flex items-center justify-between mt-8 pt-6 border-t">
                  {!isLessonComplete(activeLesson.id) ? (
                    <Button className="bg-green-600 hover:bg-green-700" onClick={function() { markLessonComplete(activeLesson.id); }}>
              ...
              <CardContent>
                {quizResult ? (
                  <div className="text-center py-8">
                    {quizResult.passed ? (
                      <div>
                        ...
                        <Button className="bg-orange-600 hover:bg-orange-700" onClick={function() { navigate('/university'); }}>
                          View Certificates
                        </Button>
                      </div>
                    ) : (
                      <div>
                        ...
                        <p className="text-gray-600 mb-4">You scored {quizResult.score}%</p>
                        <Button variant="outline" onClick={function() { setQuizResult(null); setQuizAnswers(new Array(course.quiz.length).fill(-1)); }}>
                          Retry
                        </Button>
                      </div>
                    )}
                  ...
                  <div className="space-y-6">
                    {course.quiz && course.quiz.map(function(question, qIndex) {
                      return (
                        <div key={question.id} className="p-4 bg-gray-50 rounded-lg">
                          ...
                          <div className="space-y-2">
                            {question.options.map(function(option, oIndex) {
                              var optClass = quizAnswers[qIndex] === oIndex 
