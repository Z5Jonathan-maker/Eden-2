    ...
    <div className="h-screen flex flex-col bg-zinc-950" data-testid="harvest-page">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col">
        ...
        <TabsContent value="map" className="flex-1 m-0 relative">
          ...
          {canManageTurf && (
            <div className="absolute top-14 left-3 z-[1000] w-72 bg-zinc-900/90 backdrop-blur border border-zinc-700/60 rounded-xl shadow-lg p-2.5 space-y-1.5">
              ...
              <div className="grid grid-cols-2 gap-1 text-[11px]">
                ...
                <span className="text-zinc-200 text-right">
                  {getRoleValue(user?.role) || 'unknown'}
                </span>
                <span className="text-zinc-500">Turfs</span>
                <span className="text-zinc-200 text-right">{territories.length}</span>
                <span className="text-zinc-500">Assignable Reps</span>
                <span className="text-zinc-200 text-right">
                  {loadingTurfUsers ? 'Loading...' : turfUsers.length}
                </span>
                <span className="text-zinc-500">Drop Mode</span>
                <span className="text-zinc-200 text-right">
                  {selectedDropTerritoryId === 'auto' ? 'Auto' : 'Manual'}
                </span>
                <span className="text-zinc-500">Last Assign</span>
                <span
                  className="text-zinc-200 text-right truncate"
                  title={lastTurfMatch?.assigned || 'None'}
                >
                  {lastTurfMatch?.assigned || 'None'}
                </span>
                <span className="text-zinc-500">Fallback</span>
                <span
                  className="text-zinc-200 text-right truncate"
                  title={lastTurfMatch?.fallbackAssigned || 'None'}
                >
                  {lastTurfMatch?.fallbackAssigned || 'None'}
                </span>
                <span className="text-zinc-500">Last Drop</span>
                <span
                  className="text-zinc-200 text-right truncate"
                  title={
                    lastTurfMatch
                      ? `${formatCoord(lastTurfMatch.lat, 5)}, ${formatCoord(lastTurfMatch.lng, 5)}`
                      : 'N/A'
                  }
                >
                  {lastTurfMatch
                    ? `${formatCoord(lastTurfMatch.lat, 4)}, ${formatCoord(lastTurfMatch.lng, 4)}`
                    : 'N/A'}
                </span>
                <span className="text-zinc-500">Nearest Turf</span>
                <span
                  className="text-zinc-200 text-right truncate"
                  title={lastTurfMatch?.nearestName || 'N/A'}
                >
                  {lastTurfMatch?.nearestName || 'N/A'}
                </span>
                <span className="text-zinc-500">Nearest Dist</span>
                <span className="text-zinc-200 text-right">
                  {Number.isFinite(coerceNumber(lastTurfMatch?.nearestMiles))
                    ? `${safeFixed(lastTurfMatch?.nearestMiles, 2)} mi`
                    : 'N/A'}
                </span>
                <span className="text-zinc-500">Pins Renderable</span>
                <span className="text-zinc-200 text-right">
                  {renderablePinCount}/{pins.length}
                </span>
                <span className="text-zinc-500">Recovered Coords</span>
                <span
                  className={`text-right ${recoveredPinCount > 0 ? 'text-cyan-300' : 'text-zinc-200'}`}
                >
                  {recoveredPinCount}
                </span>
                <span className="text-zinc-500">Invalid Pins</span>
                <span
                  className={`text-right ${invalidPinCount > 0 ? 'text-red-300' : 'text-zinc-200'}`}
                >
                  {invalidPinCount}
                </span>
                <span className="text-zinc-500">Unsynced Queue</span>
                <span
                  className={`text-right ${unsyncedCount > 0 ? 'text-amber-300' : 'text-zinc-200'}`}
                >
                  {unsyncedCount}
                </span>
                <span className="text-zinc-500">Session Drops</span>
                <span className="text-zinc-200 text-right">{pinHealthSession.dropped}</span>
                <span className="text-zinc-500">Session Failures</span>
                <span
                  className={`text-right ${pinHealthSession.failed > 0 ? 'text-red-300' : 'text-zinc-200'}`}
                >
                  {pinHealthSession.failed}
                </span>
                <span className="text-zinc-500">GPS Fix</span>
                <span className={`text-right ${gpsStale ? 'text-amber-300' : 'text-zinc-200'}`}>
                  {gpsStale ? 'Stale' : 'Live'}
                </span>
                <span className="text-zinc-500">GPS Accuracy</span>
                <span
                  className={`text-right ${Number.isFinite(gpsAccuracyMeters) && gpsAccuracyMeters > GPS_MAX_ACCURACY_METERS ? 'text-amber-300' : 'text-zinc-200'}`}
                >
                  {Number.isFinite(gpsAccuracyMeters) ? `${Math.round(gpsAccuracyMeters)}m` : 'N/A'}
                </span>
              </div>
              {turfUsersError && (
                <p className="text-[10px] text-red-300 font-mono truncate" title={turfUsersError}>
                  Reps API: {turfUsersError}
                </p>
              )}
              {invalidPinSamples.length > 0 && (
                <div className="rounded-lg border border-red-500/20 bg-red-500/5 p-2">
                  <p className="text-[10px] uppercase tracking-wide text-red-300 font-mono mb-1">
                    Invalid pin samples
                  </p>
                  {invalidPinSamples.map((entry) => (
                    <p
                      key={`invalid-pin-${entry.id}`}
                      className="text-[10px] text-zinc-300 truncate"
                      title={`${entry.id} (${entry.source}) ${entry.label}`}
                    >
              ...
              <button
                onClick={handleRepairInvalidPins}
                disabled={repairingPins}
                className="mt-1 w-full rounded-lg border border-cyan-500/30 bg-cyan-500/10 px-2 py-1.5 text-[11px] font-mono text-cyan-300 hover:bg-cyan-500/20 disabled:opacity-50 disabled:cursor-not-allowed"
                data-testid="repair-invalid-pins-btn"
              >
                {repairingPins ? 'Repairing Pins...' : 'Repair Invalid Pins'}
              </button>
              {unresolvedPinIds.length > 0 && (
                <div className="mt-1 rounded-lg border border-amber-500/25 bg-amber-500/5 p-2">
                  <div className="flex items-center justify-between">
                    <p className="text-[10px] font-mono uppercase tracking-wide text-amber-300">
                      Unresolved After Repair ({unresolvedPinIds.length})
                    </p>
                    <button
                      onClick={() => setShowUnresolvedPins((prev) => !prev)}
                      className="text-[10px] text-amber-200 underline"
                    >
                      {showUnresolvedPins ? 'Hide' : 'Show'}
                    </button>
                  </div>
                  <button
                    onClick={() => focusUnresolvedPin(unresolvedPinIds[0])}
                    className="mt-1 w-full rounded-lg border border-amber-400/30 bg-amber-500/10 px-2 py-1 text-[10px] font-mono text-amber-200 hover:bg-amber-500/20"
                  >
                    Review First Unresolved
                  </button>
                  {showUnresolvedPins && (
                    <div className="mt-1 max-h-24 overflow-y-auto space-y-1 pr-1">
                      {unresolvedPinIds.slice(0, 20).map((pinId) => (
                        <button
                          key={`unresolved-${pinId}`}
                          onClick={() => focusUnresolvedPin(pinId)}
                          className="w-full rounded border border-zinc-700 bg-zinc-800 px-2 py-1 text-left text-[10px] text-zinc-200 hover:border-amber-400/40"
                          title={pinId}
                        >
                          {pinId}
                        </button>
              ...
              <button
                ...
                disabled={!isOnline || unsyncedCount <= 0 || syncingQueueNow}
                className="mt-1 w-full rounded-lg border border-emerald-500/30 bg-emerald-500/10 px-2 py-1.5 text-[11px] font-mono text-emerald-300 hover:bg-emerald-500/20 disabled:opacity-50 disabled:cursor-not-allowed"
                data-testid="sync-queued-pins-btn"
              >
                {syncingQueueNow ? 'Syncing Queue...' : `Sync Queued Pins (${unsyncedCount})`}
              </button>
              <label className="mt-1 flex items-center justify-between rounded-lg border border-zinc-700/60 bg-zinc-800/50 px-2 py-1.5 text-[11px] font-mono text-zinc-300">
                <span>Auto-repair invalid pins</span>
                <input
                  type="checkbox"
                  checked={autoRepairInvalidPins}
                  onChange={(e) => setAutoRepairInvalidPins(e.target.checked)}
          ...
          {!canManageTurf && (
            <div className="absolute top-14 left-3 z-[1000] w-64 bg-zinc-900/90 backdrop-blur border border-zinc-700/60 rounded-xl shadow-lg p-2.5 space-y-1.5">
              <p className="text-[10px] font-mono uppercase tracking-wide text-zinc-500">
                Pin Health
              </p>
              <div className="grid grid-cols-2 gap-1 text-[11px]">
                <span className="text-zinc-500">Renderable</span>
                <span className="text-zinc-200 text-right">
                  {renderablePinCount}/{pins.length}
                </span>
